...
| インデント0の `2-3.` `2-3. 部屋全体が映る見開き` など | 見開き（2〜3ページ目にまたがるページ区切り） | （見出し自体は出力しない） |
...
- 見開きは `開始-終了.` の形（半角ハイフン）で書きます。範囲内のページ番号はすべて使用済みとして扱うので、
  `2-3.` の次は `4.` から続けます。`3-2.` や `2-2.` のように終了が開始以下の範囲は警告して単ページとして扱い、
  `2-4.` のように3ページ以上の範囲は受け付けつつ警告します
- 右綴じの漫画では見開きは偶数ページから始まる（`2-3.` `4-5.` …）ので、`3-4.` のように
  奇数ページから始まる見開きは警告します。見開きとしての扱いは変わりません
...
4. クリックしてモーダルが開き、以下になることを確認する
   ...
   - 2ページ目: 抽出項目0件
   - 3-4ページ目: 見出しが「3-4ページ目（見開き） — 部屋全体が映る見開き」で、セリフ1項目
...
```json
{
  ...
  "pages": [
    ...
    { "number": 2, "endNumber": 3, "label": "見開き", "items": [], "warnings": [] }
  ],
  "warnings": []
}
```
...
- `number` は開始ページ番号、`endNumber` は終了ページ番号です。単ページなら両者は同じ値で、
  見開き `2-3.` なら `number: 2, endNumber: 3` になります。
- `pageCount` は抽出したページ区切りの個数です。最大ページ番号ではなく、見開きも1つと数えます。
...
単体テストは種別・元行番号・警告・空ページ・番号・見開きの範囲の保持、JSONの往復、
